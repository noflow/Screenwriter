...
assert.equal(backlog.room_scope.completed_locations.length, 43);
assert.equal(backlog.audio_in_scope, false);
assert.equal(backlog.phases.length, 37);
...
assert.deepEqual(summary, {
  ...
  backgrounds:243,
  portraits:0,
  statuses:{missing:0,placeholder:0,in_progress:207,review:0,ready:36}
});
...
assert.match(context.ALL_MARKUP, /243 prioritized assets/);
