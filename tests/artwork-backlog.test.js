...
assert.equal(backlog.room_scope.completed_locations.length, 44);
assert.equal(backlog.audio_in_scope, false);
assert.equal(backlog.phases.length, 38);
...
assert.deepEqual(summary, {
  ...
  backgrounds:254,
  portraits:0,
  statuses:{missing:0,placeholder:0,in_progress:218,review:0,ready:36}
});
...
assert.match(context.ALL_MARKUP, /254 prioritized assets/);
