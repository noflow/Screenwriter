...
assert.equal(backlog.room_scope.completed_locations.length, 16);
assert.equal(backlog.audio_in_scope, false);
assert.equal(backlog.phases.length, 10);
...
assert.deepEqual(summary, {
  ...
  backgrounds:95,
  portraits:0,
  statuses:{missing:0,placeholder:0,in_progress:72,review:0,ready:23}
});
...
assert.match(context.ALL_MARKUP, /95 prioritized assets/);
