...
assert.equal(backlog.room_scope.completed_locations.length, 61);
assert.equal(backlog.audio_in_scope, false);
assert.equal(backlog.phases.length, 55);
...
assert.deepEqual(summary, {
  ...
  backgrounds:341,
  portraits:0,
  statuses:{missing:0,placeholder:0,in_progress:305,review:0,ready:36}
});
...
assert.match(context.ALL_MARKUP, /341 prioritized assets/);
